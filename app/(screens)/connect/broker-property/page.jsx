...
export default function ShortlistPage() {
  ...
  const getBrokerProperties = async ()=>{
    
    
    try {
        const response = await axios.get(`/api/getListings_Agent`);
      console.log(response);
      
        
        if(response.status === 200){
            setBrokerProperty(response.data.listings);
...
//   const filteredProperties = brokerProperty?.filter((property) => {
//     const isShortlisted = shortlistType ? property?.shortlistedBy === shortlistType : true;
//     const matchesLocation = searchLocation ? property?.location?.toLowerCase().includes(searchLocation.toLowerCase()) : true;

//     const matchesBudget =
//       budget && property?.price
//         ? property.price.replace(/[^0-9.]/g, "").includes(budget.replace(/[^0-9.]/g, ""))
//         : true;
//     const matchesBHK = bhk ? Number(property?.bhk) === Number(bhk) : true;
//     const matchesType = propertyType ? property?.type?.toLowerCase() === propertyType.toLowerCase() : true;
  
//     return isShortlisted && matchesLocation && matchesBudget && matchesBHK && matchesType;
//   });
   
const filteredProperties = brokerProperty
  ...
  <div key={property.id} className="bg-white p-4 rounded shadow mb-4 relative">
    ...
    <div className="flex flex-row">
      <Image
        src={property.image?.[0] || "https://img.squareyards.com/secondaryPortal/IN_638698620300780078-1512241220302030.jpg?aio=w-360;h-210;crop"}
        alt={property.title}
        width={200}
        height={200}
      ...
      <div className="relative">
        ...
        >
          {property.source !== "" && 
            <Image
              key={index}
              src={`/images/${property.source}.png`} 
              alt={src}
              width={24}
              height={24}
